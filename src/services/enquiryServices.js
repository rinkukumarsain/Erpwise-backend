...
exports.updateLeadById = async (auth, leadId, updatedData, orgId) => {
    try {
        ...
        const data = await query.findOne(enquiryModel, { _id: leadId, isDeleted: false });
        if (!data) {
            return {
                success: false,
        ...
        if (updatedData.companyName) {
            const findUniqueCompanyName = await query.findOne(enquiryModel, { organisationId: orgId, companyName: updatedData.companyName, _id: { $ne: leadId } });
        ...
        const updatedLead = await enquiryModel.findByIdAndUpdate(
            leadId,
            updatedData,
...
exports.delete = async (leadId, auth) => {
    try {
        ...
        // }
        // arr.push(enquiryModel.findByIdAndDelete(leadId));
        ...
        updatedData['$push'] = { Activity: obj };
        const deleteLead = await enquiryModel.findByIdAndUpdate(
            leadId,
            updatedData
        );
        if (deleteLead) {
...
exports.createProspect = async (auth, prospectData, orgId) => {
    try {
        ...
        const findUniqueCompanyName = await query.findOne(enquiryModel, { organisationId: orgId, companyName: prospectData.companyName });
        if (findUniqueCompanyName) {
            return {
                success: false,
                message: 'Company name already exist.'
        ...
        const newLead = await query.create(enquiryModel, prospectData);
        return {
            success: true,
...
exports.addLeadFinance = async (auth, financeData, leadId, orgId) => {
    try {
        const findLead = await query.findOne(enquiryModel, { _id: leadId, organisationId: orgId, isDeleted: false });
        if (!findLead) {
            return {
                success: false,
                message: 'Lead not found.'
        ...
        updatedData['$push'] = { Activity: obj };
        const updatedLeadFinance = await enquiryModel.findByIdAndUpdate(
            leadId,
            updatedData,
            { new: true, runValidators: true }
        );
...
exports.uploadLeadDocument = async (leadId, { location }, auth) => {
    try {
        ...
        const findAndUpdateLeadDocument = await enquiryModel.findOneAndUpdate({ _id: leadId, isDeleted: false }, { $push: { documents: location, Activity: obj }, updatedBy: auth._id }, { new: true });
...
exports.deleteLeadDocument = async (leadId, imageUrl, auth) => {
    try {
        ...
        const findAndUpdateLeadDocument = await enquiryModel.findOneAndUpdate({ _id: leadId, isDeleted: false }, { $pull: { documents: imageUrl }, $push: { Activity: obj } }, { new: true });
