    ...
    try {
        ...
        if (reqBody.startDate) {
            dateCheck = await query.findOne(exchangeRateModel, {
                $expr: {
                    $and: [
                        { $ne: [new mongoose.Types.ObjectId(reqBody._id), '$_id'] },
                        { $lte: [new Date(reqBody.startDate), '$endDate'] },
                        { $gte: [new Date(reqBody.startDate), '$startDate'] }
                    ]
                }
        ...
        const data = await exchangeRateModel.findByIdAndUpdate({ _id: reqBody._id }, reqBody, { new: true });
        return {
            success: true,
