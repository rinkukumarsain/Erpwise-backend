...
const { shipmentLevel } = require('../../config/default.json');
...
/**
 * Edit enquiry item shipment by id to accept warehouse goods for Shipment Dispatched
 *
...
exports.AcceptTheGoodsGI = async (enquiryId, shipmentId, orgId, body, auth) => {
    try {
        ...
        if (findShipment.level != 2) {
            const text = shipmentLevel[2]?.split('_').join(' ');
            return {
                success: false,
                message: `Goods can only be accepted at level ${text}`
            };
        }
        ...
        return {
            success: false,
            message: `Error while accepting warehouse goods.`
        };
    } catch (error) {
